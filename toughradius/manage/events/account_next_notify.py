...
class AccountNextNotifyEvent(BasicEvent):
    ...
    MAIL_TPLNAME = 'tr_renew_notify'
    MAIL_APIURL = "%s/sendmail" % toughcloud.apiurl

    SMS_TPLNAME = 'tr_renew_notify'
    SMS_APIURL = "%s/sendsms" % toughcloud.apiurl

    @defer.inlineCallbacks
    def event_toughcloud_sms_account_next(self, userinfo):
        """ toughCloud sms api user next notify event """
        if not userinfo:
            return
    ...
    @defer.inlineCallbacks
    def event_toughcloud_mail_account_next(self, userinfo):
        """ toughCloud mail api user next notify event """
        if not userinfo:
            return
    ...
    def event_smtp_account_next(self, userinfo):

        open_notify = u"""尊敬的 %customer% 您好：
         您的宽带账号 %username% 已续费产品 %product%, 服务截止至 %expire%。"""
